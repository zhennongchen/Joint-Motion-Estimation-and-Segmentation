...
main_folder = os.path.join(defaults.sam_dir, 'models/unet2D_LSTM_trial2_alldata/predicts')
epoch = 70
...
for i in range(0,len(patient_id_list)):
    ...
    for slice_num in np.arange(1, len(gt_files) - 1):
        ...
        pred2 = nb.load(pred_file2).get_fdata()  # for HD calculation
    ...
    result.append([patient_id, all_dice, base_dice, mid_dice, apex_dice, all_hd, base_hd, mid_hd, apex_hd])
...
df = pd.DataFrame(result, columns = ['patient_id', 'all_dice', 'base_dice', 'mid_dice', 'apex_dice', 'all_hd', 'base_hd', 'mid_hd', 'apex_hd'])
