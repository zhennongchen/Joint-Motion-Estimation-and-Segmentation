...
class Dataset_CMR(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        processed_image_torch = torch.from_numpy(np.rollaxis(processed_image, 2, 0)).unsqueeze(0).float()
        processed_seg_torch = torch.from_numpy(np.rollaxis(processed_seg, 2, 0)).unsqueeze(0)  

        # also need to return the original image and seg without the augmentation (with center crop done)
        original_image_torch = torch.from_numpy(np.rollaxis(original_image, 2, 0)).unsqueeze(0).float()
        original_seg_torch = torch.from_numpy(np.rollaxis(original_seg, 2, 0)).unsqueeze(0)
